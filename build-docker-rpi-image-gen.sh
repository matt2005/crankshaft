...
# Crankshaft Docker Build Script using rpi-image-gen
# This script builds Crankshaft images using a modern Docker-based approach

DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null 2>&1 && pwd )"

DOCKER="docker"
CONTAINER_NAME="crankshaft-builder"
...
do
	case "${flag}" in
		...
		h)
			...
			echo "  -r RELEASE   Debian release (trixie, bookworm)"
			echo "  -n NAME      Image name"
			echo "  -v           Verbose output"
			echo "  -h           Show help"
			echo ""
			echo "Examples:"
			echo "  $0 -c config -a arm64 -r trixie"
...
echo "Building Crankshaft with rpi-image-gen"
echo "======================================"
